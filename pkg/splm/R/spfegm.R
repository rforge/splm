`spfegm` <-
function(formula, data=list(), index=NULL, w, method = c("init","fulweigh"), effects=c('pooled','spfe','tpfe','sptpfe'), lag=FALSE, endog = NULL, instruments= NULL, verbose = FALSE, control = list()){
 ## depends on:
#source('args.R')
...
if(!is.null(endog)) fortrans<- cbind(x[,-xend], Kmat)
else fortrans<-x	
...
if (effects=="tpfe" | effects=="sptpfe"){
	...
	xtms<-apply(fortrans,2,tpms)   ###same thing for the X variable
...
if (effects=="sptpfe"){ ####generate the demeaned variables for both types of FE
	...
	xmm<-matrix(,NT,(k))
	for (i in 1:(k)) xmm[,i]<-rep(mean(fortrans[,i]),NT)
	xt<- fortrans - xsm - xtm + xmm
...
##moments procedure
  ...
  switch(method, init = {
	...
	finsigmaV=estim1$par[2]
      ...
      estim2 <- nlminb(pars2, arg3, v = Gg, ss=estim1$par[2], T=T,TW = weights$TW ,verbose = verbose, control = control, lower=c(-0.999,0), upper=c(0.999,Inf))
