...
function(X, y, ind, tind, n, k, t, nT, w=NULL, coef0=0,
  ...
  ## time- error autoregressive, (pooling) panel model estimation
  ## modified from ssrREmod.R

  ## w included only for parameters compatibility with splm(), but
  ## obviously not used!

  ## reversed data ordering (unaffected, see cfr. ssrREmod()).

  ## checked against gls(fm, data, correlation = corAR1(0, form = ~ tind|ind))
  ## from 'nlme' and looks like working.

  ## Giovanni Millo, Trieste; this 'splm' version: 23/10/2008.
  ...
  ## concentrated likelihood
    ll.c<-function(rho, y, X, n, t, w) {
            ...
            uno <- n/2*log(1-rho^2)
            tre <- -(n*t)/2*log(s2e)
            ## use only the TxT block (Vrho)^(-1) and calculate block by block
  ...
  optimum<-nlminb(myrho0, ll.c,
                  lower=c(-0.999), upper=c(0.999),
  ...
  ## calc. cov(b) by GLS
  covB<-as.numeric(beta[[3]]) * solve(crossprod(X, solve(fullSigma(rho, n, t), X)))
  dimnames(covB) <- list(nam.beta, nam.beta)

  ## calc. cov(rho) by numerical Hessian
  covPRL<-solve(-fdHess(myrho, function(x) -ll.c(x,y,X,n,t,w))$Hessian)
