sarpanelerror<-function (lambda, eig, y, wy, x, WX, n, NT,T,quiet) 
{
    yl <- y - lambda * wy
    xl <- x - lambda * WX
    xl.q <- qr.Q(qr(xl))
...
sarpanelerror.sp<-function (lambda, csrw, I, y, wy, x, WX, n,NT,T, quiet) 
{
    yl <- y - lambda * wy
    xl <- x - lambda * WX
    xl.q <- qr.Q(qr(xl))
...
sarpanelerror.M<-function (lambda, csrw, I, y, wy, x, WX, n,NT,T, quiet) 
{
    yl <- y - lambda * wy
    xl <- x - lambda * WX
    xl.q <- qr.Q(qr(xl))
