`spfeml` <-
function(formula, data, index=NULL,listw, model=c("lag","error"),effects=c('pooled','spfe','tpfe','sptpfe'), method="eigen",na.action=na.fail,quiet=TRUE,zero.policy = FALSE,
...
#source("likelihoodslag.R")
#source("likelihoodserror.R")
#source("dosparsepanel.R")
#source("spfeml.R")
#source("splaglm.R")
#source("utilities.R")
#source("sperrorlm.R")
...
if(model=='lag'){
   RES <- splaglm(xt,yt,wyt,listw,method,K=k,NT=NT,T=T,N=N,inde,zero.policy = FALSE,quiet=quiet,
